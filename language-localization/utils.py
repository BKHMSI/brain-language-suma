...
def get_num_blocks(model_name, num_blocks):
    if "suma-1" in model_name or "random" in model_name:
        return 1
    return {
        "gpt2": 12,
        "gpt2-medium": 24,
...
def get_layer_names(model_name, num_blocks=None):
    ...
    elif "Llama-2" in model_name or model_name == "suma":
    ...
    elif model_name == "suma-1-ln-attn-p2":
    ...
    elif model_name == "suma-simple" or model_name == "suma-1-ln-attn" or model_name == "suma-1-l-attn" or model_name == "suma-1-ln-attn-pos":
        return [f'layers.{layer_num}.{layer_desc}' 
            for layer_num in range(num_blocks) 
            for layer_desc in ["input_layernorm", "self_attn"]
        ]
    elif model_name == "suma-1-attn":
        return [f'model.layers.{layer_num}.{layer_desc}' 
            for layer_num in range(num_blocks) 
            for layer_desc in ["self_attn"]
        ]
    elif model_name == "suma-1-mlp":
        return [f'layers.{layer_num}.{layer_desc}' 
            for layer_num in range(num_blocks) 
            for layer_desc in ["mlp"]
        ]
    elif model_name == "suma-1-ln-mlp":
        return [f'layers.{layer_num}.{layer_desc}' 
            for layer_num in range(num_blocks) 
            for layer_desc in ["input_layernorm", "mlp"]
        ]
    elif model_name == "suma-1-ln-attn-ln-mlp" or model_name == "suma-1-pos-ln-attn-ln-mlp" or "suma-1" in model_name:
        return [f'layers.{layer_num}.{layer_desc}' 
            for layer_num in range(num_blocks) 
            for layer_desc in ["input_layernorm", "self_attn", "post_attention_layernorm", "mlp"]
        ]
